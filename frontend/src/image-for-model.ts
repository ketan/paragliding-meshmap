...
const allImages = import.meta.glob('./assets/images/devices/*.svg', { eager: true, query: '?url', import: 'default' })
const hardwareModelLookup: Record<string, string> = {}

for (const path in allImages) {
  const imageNameWithoutExtension = path.split('/').at(-1)?.replace('.svg', '')
  if (allImages[path]) {
    const modelName = findKey(hardwareModelToFilename, (v) => v === `${imageNameWithoutExtension}.svg`)
    if (modelName) {
      hardwareModelLookup[modelName] = allImages[path] as string
    }
  }
}
...
export function imageForModel(modelId?: number) {
  if (modelId) {
    const modelName = HardwareModelIDToName[modelId]
    return hardwareModelLookup[modelName] || hardwareModelLookup['UNKNOWN']
  }
  return hardwareModelLookup['UNKNOWN']
}
